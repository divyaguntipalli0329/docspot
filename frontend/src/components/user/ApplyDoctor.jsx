...
function ApplyDoctor({ userId }) {
   ...
   const handleTimingChange = (values) => {
      // Convert moment objects to "HH:mm" strings
      setDoctor({ ...doctor, timings: values ? values.map(val => dayjs(val).format('HH:mm')) : [] });
   };
   ...
   const handleSubmit = async () => {
      try {
         const res = await axios.post('http://localhost:8001/api/user/registerdoc', { doctor, userId }, {
            headers: {
               ...
               <Col xs={24} md={12} lg={8}>
                  <Form.Item label="Timings" name="timings" required>
                     <TimePicker.RangePicker
                        format="HH:mm"
                        value={doctor.timings.length === 2 ? [dayjs(doctor.timings[0], 'HH:mm'), dayjs(doctor.timings[1], 'HH:mm')] : []}
